...
/**
 * Verify Forgot Password OTP Form Validation Schema
...
type VerifyForgotPasswordFormData = z.infer<typeof verifyForgotPasswordSchema>
...
export function VerifyForgotPasswordForm({
  ...
  useEffect(() => {
    ...
    if (!email) {
      toast.error("No email found. Please request password reset again.")
   ...
   * Uses auth service directly for verify forgot password OTP
   */
  const onSubmit = async (data: VerifyForgotPasswordFormData) => {
    try {
      ...
      const response = await authService.verifyForgotPasswordOtp(data,userEmail)
  ...
  /**
   * Handle Resend OTP
   * Resends OTP to user's email for password reset
   */
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter the 6-digit verification code sent to your email address for password reset
            {userEmail && (
